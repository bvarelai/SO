...
void comando(char *string, char *number, bool end, tList L, tListM L2);
void ayuda(char *cmd);
void imprimirPromp();
void leerEntrada(char *string, char *chunks[], tList *L);
void ProcesarCadena(char *string, char *chunks[], bool *end, tList *L, tListM *L2);
...
void comando(char *string, char *number, bool end, tList L, tListM L2)
{
   ...
   else if (isdigit(*number))
   {
      ...
      else
      {
         ...
         ProcesarCadena(string, cut_up, &end, &L, &L2);
      }
   }
   else
